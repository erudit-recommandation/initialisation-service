...
path_df_som_grid = os.path.join(directory,'doc_countvectors', 'df_som_grid_bmus.csv')

cols = ['title', 'idproprio', 'bmu']


path_doc_parse = os.path.join(directory, 'doc_parse.csv')
cols_doc_parse = ['author','title','titrerev','idproprio','ppage','sstitrerev','idissnnum','nonumero','theme','periode','annee','lemma','text']
...
def create_som_images(n_rows):
    imagecreated = 0
    df =  pd.read_csv(path_df_som_grid, encoding='utf-8', chunksize=chunksize, sep=',',usecols=cols)
    
    df_bonified_doc_parse = pd.read_csv(path_doc_parse, encoding='utf-8',nrows=n_rows,sep=';',lineterminator='\n',usecols=cols_doc_parse)
    
    df_bonified_doc_parse = df_bonified_doc_parse.assign(bmu=bmus,
                            som_persona= [som_grid.codebook.matrix[bmu].reshape(50,50) for bmu in bmus],
                            )
